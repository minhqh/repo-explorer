...
export function generateContextPack(data: RepositoryData): string {
    ...
    const entryFileNames = ['main.py', 'app.py', 'server.js', 'index.js', 'main.tsx', 'App.tsx', 'index.ts'];
    ...
    const IGNORED_EXTENSIONS = ['.png', '.jpg', '.jpeg', '.svg', '.ico', '.gif', '.mp4', '.pdf', '.lock', '.woff', '.ttf'];
    ...
    if (data.git_stats && data.git_stats.recent_commits.length > 0) {
        ...
        const others = commits.filter(c => !c.message.toLowerCase().includes('feat') && !c.message.toLowerCase().includes('fix')).slice(0, 5); // Lấy tượng trưng
