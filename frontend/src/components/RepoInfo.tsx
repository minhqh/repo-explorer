    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
        <img 
          ...
          alt={info.owner} 
          style={{ width: '64px', height: '64px', borderRadius: '50%', border: '1px solid #d0d7de' }}
        />
        <div>
          <h2 style={{ margin: 0, fontSize: '1.5rem', color: '#0969da' }}>
            <a href={`https://github.com/${info.owner}/${info.name}`} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', color: 'inherit' }}>
      ...
      <div style={{ display: 'flex', gap: '16px', fontSize: '14px', color: '#57606a' }}>
        <span>⭐ <strong>{info.stars.toLocaleString()}</strong> stars</span>
        <span>🍴 <strong>{info.forks.toLocaleString()}</strong> forks</span>
        {info.language && <span>🔵 <strong>{info.language}</strong></span>}
      </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {info.topics.map(topic => (
            <span key={topic} style={{
              ...
              padding: '4px 10px', borderRadius: '2em', fontSize: '12px', fontWeight: 500
            }}>
              {topic}
            </span>
          ))}
